...
def CannyThreshold(val):
    ...
    for c in contours:
        ...
        polys.append(cv.approxPolyDP(c, cv.getTrackbarPos(polys_trackbar, window_name), True))
    ...
    if len(hulls) > 1:
        for i, h1 in enumerate(hulls):
            for j, h2 in enumerate(hulls):
                if h1 is h2:
                    continue
    ...
    # Remove all nested hulls.
    if len(nested_hull_indices) > 0:
        for i in nested_hull_indices:
            hulls[i] = None
        else:
...
cv.createTrackbar(frame_trackbar, window_name, 0, int(vid.get(cv.CAP_PROP_FRAME_COUNT)) - 1, CannyThreshold)
...
cv.setTrackbarPos(canny_trackbar, window_name, 50)  # This seems to be a nice default (for test_02 anyway)
